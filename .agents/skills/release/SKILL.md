---
name: release
description: Prepare, authorize, publish, troubleshoot, and verify Cursor BYOK desktop packages and GitHub Releases. Use for Windows packaging, version bumps, release tags, GitHub Actions release runs, signing, or release-readiness checks.
---

# Desktop release

Release installers through `.github/workflows/release.yml`. The application has no in-app update checks or installation support. Keep updater artifacts, manifests, plugins, and permissions disabled.

## Local Windows package

A local package does not authorize a GitHub release, version bump, installation, or application restart. Preserve the current version and all user changes unless instructed otherwise.
...
The Windows x64 installer is written under `target/release/bundle/nsis/` at the workspace root. Local packaging requires no updater signing key.
...
- Without that authorization, restrict work to inspection, local edits, validation, and local packages. Do not infer publication permission from requests such as “prepare”, “check”, or “ready to release”.
- Never print, commit, or upload existing signing private keys.
...
3. Confirm `createUpdaterArtifacts` is false in `tauri.conf.json` and updater endpoints and permissions are absent.
4. Confirm neither the intended tag nor Release already exists.
5. From `apps/desktop`, run:
...
6. Validate the workflow YAML and inspect the staged diff. Ensure `.tauri/`, unrelated local files, and unrelated user changes are not staged.
7. Keep `uploadUpdaterJson: false` in `tauri-action@v1`. Release only ordinary platform packages.
...
5. Verify the Release contains the expected Windows, macOS, and Linux packages without update manifests or updater signatures.
